...
static Limine_MemMap *create_mem_map(int entry_count) {
    Limine_MemMap *map = munit_malloc(sizeof(Limine_MemMap) +
                                      sizeof(Limine_MemMapEntry) * entry_count);
...
static MunitResult
test_init_one_bootloader_reclaimable(const MunitParameter params[],
                                     void *param) {
    Limine_MemMapEntry entry = {.type = LIMINE_MEMMAP_BOOTLOADER_RECLAIMABLE,
                                .base = 0x0000000000000000,
                                .length = 0x0000000000100000};
...
static const MunitSuite test_suite = {(char *)"/pmm/stack_limine",
