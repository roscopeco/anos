...
void vmm_map_page(uint64_t *pml4, uintptr_t virt_addr, uint64_t page,
  ...
  // If we don't have a PML4 entry at this index - create one, and a PDPT for it
  // to point to
  uint64_t *pdpt =
      ensure_table_entry(pml4, PML4ENTRY(virt_addr), PRESENT | WRITE);
  ...
  // to point to
  uint64_t *pd =
      ensure_table_entry(pdpt, PDPTENTRY(virt_addr), PRESENT | WRITE);
  ...
  // If we don't have a PD entry at this index - create one, and a PT for it to
  // point to
