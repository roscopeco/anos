...
static void copy_page_safely(const uintptr_t src_virt_page,
    ...
    vmm_map_page(per_cpu_temp_page, dest_phys_page, PG_PRESENT | PG_WRITE);
...
void page_fault_handler(const uint64_t code, const uint64_t fault_addr,
    ...
    const uintptr_t current_phys_addr = pte & PAGE_ALIGN_MASK;
    ...
    if (region && region->flags & VM_REGION_AUTOMAP) {
        ...
        vmm_map_page(fault_addr_page, kernel_zero_page,
                     PG_USER | PG_PRESENT | PG_COPY_ON_WRITE);
        return;
    }
