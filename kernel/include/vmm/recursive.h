...
// Base address for tables (high bits always set, tables will always be in
// kernel space...)
...
// `0xffff800000000000`-> `0xffff807fffffffff` : 512GB Recursive mapping area
// when @ PML4[256]
...
// Fixed parts of addresses used when building table access addresses for
// virtual addresses at various levels. These are all precomputed and should
// just end up as constants...
//
...
/*
 * Build a virtual address to access a specific page table and (byte) offset
 * within recursive mappings. This is kinda low-level, the other `vmm_recursive`
 * functions build on this and should usually be used instead.
 *
 * The tests have some example usage, but generally you use this by specifying
 * the entry to use for each level of the page tables, repeating the recursive
 * mapping index as often as needed for the appropriate table level you're
 * after.
 *
 * For example, to get the PML4 itself, just specify the recursive mapping for
 * all levels, and a zero offset:
 *
 * ```C
 ...
 * ```
 *
 * Note that this function **does not** canonicalise addresses automatically -
 * so the l1 value must translate to addresses above 0xffff800000000000.
 *
 * In practice, this means that the recursive mapping must be in the top 256
 * entries of the PML4 - the kernel maps the _current_ PML4 into the
 * second-from-top (index 510) entry, and I plan to map alternative process
 * spaces into the third-from-top (index 509) if/when that becomes a thing, so
...
/*
 * Find the PML4 using the _current process'_ recursive mapping (specified by
 * `RECURSIVE_ENTRY`).
 */
...
/*
 * Find a given PDPT using the _current process'_ recursive mapping (specified
 * by `RECURSIVE_ENTRY`).
 */
...
/*
 * Find a given PD using the _current process_ recursive mapping (specified by
 * `RECURSIVE_ENTRY`).
 */
...
/*
 * Find the PT using the _current process_ recursive mapping (specified by
 * `RECURSIVE_ENTRY`).
 */
...
/*
 * Find the PTE mapping the given virtual address using the _current process_
 * recursive mapping (specified by `RECURSIVE_ENTRY`).
 */
static inline uint64_t *vmm_virt_to_pte(uintptr_t virt_addr) {

    //                              Extend        PML4      PDPT       PD PT
    //                              Offset
    // 0x0000008080604000 :: 0b0000000000000000 000000001 000000010 000000011
    // 000000100 000000000000
    // ... becomes ...
    // 0xffff800040403020 :: 0b1111111111111111 100000000 000000001 000000010
    // 000000011 000000100000
...
/*
 * Find the PT mapping the given virtual address using the _current process_
 * recursive mapping (specified by `RECURSIVE_ENTRY`).
 */
...
/*
 * Find the PDE mapping the given virtual address using the _current process_
 * recursive mapping (specified by `RECURSIVE_ENTRY`).
 */
static inline uint64_t *vmm_virt_to_pde(uintptr_t virt_addr) {
    ...
    // 000000100 000000000000
    // ... becomes ...
    // 0xffffffffc0202018 :: 0b1111111111111111 111111111 111111111 000000001
    // 000000010 000000011000
...
/*
 * Find the PD mapping the given virtual address using the _current process_
 * recursive mapping (specified by `RECURSIVE_ENTRY`).
 */
...
/*
 * Find the PDPTE mapping the given virtual address using the _current process_
 * recursive mapping (specified by `RECURSIVE_ENTRY`).
 */
static inline uint64_t *vmm_virt_to_pdpte(uintptr_t virt_addr) {
    ...
    // 000000100 000000000000
    // ... becomes ...
    // 0xffffffffffe01010 :: 0b1111111111111111 111111111 111111111 111111111
    // 000000001 000000010000
...
/*
 * Find the PDPT mapping the given virtual address using the _current process_
 * recursive mapping (specified by `RECURSIVE_ENTRY`).
 */
...
/*
 * Find the PML4E mapping the given virtual address using the _current process_
 * recursive mapping (specified by `RECURSIVE_ENTRY`).
 */
static inline uint64_t *vmm_virt_to_pml4e(uintptr_t virt_addr) {
    ...
    // 000000100 000000000000
    // ... becomes ...
    // 0xfffffffffffff008 :: 0b1111111111111111 111111111 111111111 111111111
    // 111111111 000000001000
...
/*
 * Find the PML4 mapping the given virtual address using the _current process_
 * recursive mapping (specified by `RECURSIVE_ENTRY`).
 *
 * This is provided for completeness really, it will return the same fixed
 * address everywhere.
 */
