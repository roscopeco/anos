...
static int64_t spawn_process_via_system(const uint64_t stack_size, const uint16_t capc,
    ...
    const SyscallResult find_channel_result = anos_find_named_channel("SYSTEM::PROCESS");
    ...
    const size_t total_size = sizeof(ProcessSpawnRequest) + capabilities_size + argv_size;
    ...
    const SyscallResult spawn_result = anos_send_message(system_process_channel, PROCESS_SPAWN, total_size, buffer);
...
void spawn_xhci_driver(const uint64_t xhci_base, const uint64_t pci_config_base, const uint64_t pci_device_id) {
    ...
    const int64_t pid =
            spawn_process_via_system(0x100000, 14, xhci_caps, 4, argv);
    if (pid > 0) {
#ifdef DEBUG_BUS_DRIVER_INIT
...
void spawn_ahci_driver(const uint64_t ahci_base, const uint64_t pci_config_base, const uint64_t pci_device_id) {
#ifdef DEBUG_BUS_DRIVER_INIT
    ...
    char pci_device_id_str[32];
    snprintf(pci_device_id_str, sizeof(pci_device_id_str), "%lu", pci_device_id);

    const char *argv[] = {"boot:/ahcidrv.elf", ahci_base_str, pci_config_str, pci_device_id_str};
    ...
    const InitCapability ahci_caps[] = {
            ...
            {.capability_id = SYSCALL_ID_SLEEP, .capability_cookie = __syscall_capabilities[SYSCALL_ID_SLEEP]},
            {.capability_id = SYSCALL_ID_MAP_PHYSICAL,
    ...
    const int64_t pid = spawn_process_via_system(0x100000, 14, ahci_caps, 4, argv);
    if (pid > 0) {
#ifdef DEBUG_BUS_DRIVER_INIT
...
static int pci_initialize_driver(const uint64_t ecam_base, const uint16_t segment, const uint8_t bus_start,
    ...
    // Map the ECAM space
    const SyscallResult result = anos_map_physical(ecam_base, (void *)ECAM_BASE_ADDRESS, bus_driver.mapped_size,
...
static void pci_enumerate_all_buses(const uint64_t pci_bus_device_id) {
    // TODO maybe don't run through all the buses?
    //      should only have one root bus, can recursively scan from there...
    for (uint16_t bus = bus_driver.bus_start; bus <= bus_driver.bus_end; bus++) {
