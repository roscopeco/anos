...
void pci_write_config32(const uint64_t pci_base, const uint16_t offset, const uint32_t value) {
    volatile uint32_t *config_space = (volatile uint32_t *)pci_base;
    config_space[offset / 4] = value;
}

void pci_write_config16(const uint64_t pci_base, const uint16_t offset, const uint16_t value) {
...
bool pci_configure_msi(const uint64_t pci_base, uint8_t msi_offset, uint64_t msi_address, uint32_t msi_data) {
    if (msi_offset == 0) {
        return false;
    }
    ...
    uint16_t msi_control = pci_read_config16(pci_base, msi_offset + MSI_CAP_CONTROL);
    ...
    // Write MSI address
    pci_write_config32(pci_base, msi_offset + MSI_CAP_ADDRESS_LO, (uint32_t)(msi_address & 0xFFFFFFFF));

    if (is_64bit) {
        pci_write_config32(pci_base, msi_offset + MSI_CAP_ADDRESS_HI, (uint32_t)(msi_address >> 32));
