...
uint32_t query_devices(const DeviceQueryType query_type, const DeviceType device_type, const uint64_t device_id,
                       DeviceInfo *results, const uint32_t max_results);
void handle_device_message(const uint64_t msg_cookie, void *buffer, const size_t buffer_size);
